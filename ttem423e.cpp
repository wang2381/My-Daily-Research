...
void TTEM::delta(const int& dm, double pstate[NVT][NUMEQ], double pdstate[NVT][NUMEQ], const int& vt)
{
  ...
  if(pstate[vt][I_SOLC]<=-400000) pstate[vt][I_SOLC] = -400000;								//wsr: boundary condition
  nfert = 1.0000000;

  //added for soil hydrology model
   hydm.soil_perco (soil.pctsilt, soil.pctclay);

   hydm.lyperc[vt][dm] =hyd.daze[dm]*hydm.k1* pow((pstate[vt][I_AVLW]/soil.awcapmm[vt]), hydm.k2);

//   hydm.lyperc1[dm] = hyd.daze[dm]*hydm.k1* pow((pstate[I_AVLW1]/soil.awcapmm1), hydm.k2);
...
soil.percol(atms.rain[vt][dm],soil.snowinf[vt][dm],atms.eet[vt][dm],pstate[vt][I_AVLW],dm,vt);
...
/*
   if ((pstate[vt][I_AVLW] + soil.snowinf[vt][dm] + atms.rain[vt][dm] - atms.eet[vt][dm]
     - soil.rperc[vt][dm] - soil.sperc[vt][dm]) < 0.0)
  {
    atms.eet[vt][dm] = pstate[vt][I_AVLW] + soil.snowinf[vt][dm] + atms.rain[vt][dm]
                   - soil.rperc[vt][dm] - soil.sperc[vt][dm];
  }
*/
 if ((pstate[vt][I_AVLW1]+pstate[vt][I_AVLW2]+pstate[vt][I_AVLW3]+ soil.snowinf[vt][dm] + atms.rain[vt][dm] - atms.eet[vt][dm]
  ...
  soil.h2oyld[vt][dm] = soil.rrun[vt][dm] + soil.srun[vt][dm];
  ...
  veg.alleaf[vt] =veg.leafmxc[veg.cmnt]/(1.0 + veg.kleafc[veg.cmnt]                                                                      //kleafc is the same
               * exp(veg.cov[veg.cmnt]*pstate[vt][I_VEGC]));													// cov is -0.006808 for tundra and 0.000567 for boreal forest
  veg.lai[vt][dm] = veg.sla[veg.cmnt] * veg.alleaf[vt];


  veg.foliage[vt] = veg.alleaf[vt] / veg.leafmxc[veg.cmnt];                                   //leafmax of forest is twice as wet tundra, 287 and 126.3                       Note: for sensitivity test, the leafmax is set to 287, 660 and 																																					1060, following the LAI 0.4, 2.5 and 5.0. 
  veg.fpc[vt][dm] = 1.0;
  
  
  
  
//printf("%9.3f,     %9.3f     %9.3f\n", veg.ingpp[vt][dm],veg.leaf[vt][dm],veg.foliage[vt]);
  ...
  if (ag.state == 0)
  {
	...
	//cout << " leaf index is" << veg.leaf[vt][dm] << "stmotal is " << gv[vt] << endl;
    if (veg.ingpp[vt][dm] < 0.0) { veg.ingpp[vt][dm] = 0.0; }

    veg.inuptake[vt] = veg.nupxclm(veg.cmnt, pstate[vt][I_SM], pstate[vt][I_AVLN],
...
//printf("%9.3f,%9.3f,%9.3f,%9.3f\n",microbe.kd[vt],soil.psiplusc,microbe.rh[vt][dm],atms.rain[vt][dm]);
//printf("%d,%9.3f\n",soil.rhflag,soil.org[vt][dm].carbon);
//printf("%d,%9.3f\n",totyr,pstate[vt][I_SOLC]);


if ( atms.tpeatflag==1)
{

if(atms.tsoiloggcflag==1)													//wsr: this is for the second time slice with soil organic carbon direct intput

{

   if (totyr >= 1000)
   {
       soil.carbonthread = 15000;
    
      init_depth = orgcarbon/15000 ; 																							//wsr: initave is the average of initial carbon in 1m

       microbe.rh[vt][dm] =    ( soil.watertable) / 300 * 0.5 * microbe.rhxclm(soil.carbonthread, dq10[vt], rhmoist[vt],vt)+soil.totalch4*30/1000;	//wsr: use initial depth to give decomp
    }
...
if(totyr>=1000)
{
         ...
         microbe.rh[vt][dm] =    ( soil.watertable) / 300 * 0.5 * microbe.rhxclm(soil.carbonthread, dq10[vt], rhmoist[vt],vt)+soil.totalch4*30/1000;	
                    
                  
   }
    else
   
       {																										//wsr: when it is in the equilibrium condition, we use upland decomposition, when in the 					//																				transient condition, we use peatland decompostion, by using rhflag

           microbe.rh[vt][dm] =   microbe.rhxclm(pstate[vt][I_SOLC], dq10[vt], rhmoist[vt],vt);
           if(soil.rhflag!=1)
           {
           soil.carbonthread = 15000;																				//initial peat carbon output from upland run
           //printf("%9.3f\n",soil.carbonthread);
           }
...
if (soil.sum_depth < 0)
{
microbe.rh[vt][dm] =    ( soil.watertable) / 1000 * microbe.rhxclm(pstate[vt][I_SOLC], dq10[vt], rhmoist[vt],vt)+soil.totalch4*30/1000;

}

if (soil.sum_depth >= 0 && soil.sum_depth < 300)
{
double aa = 10000;
double delta = ( pstate[vt][I_SOLC] - aa)  + aa / 1000 * (300 - soil.sum_depth) ;
microbe.rh[vt][dm] =    ( soil.watertable) / 300 * microbe.rhxclm(delta, dq10[vt], rhmoist[vt],vt)+soil.totalch4*30/1000;
}

if (soil.sum_depth >= 300)

{
double aa = 10000;
double delta = (pstate[vt][I_SOLC] - aa ) / soil.sum_depth * 300;
microbe.rh[vt][dm] =    ( soil.watertable) / 300 * microbe.rhxclm(delta, dq10[vt], rhmoist[vt],vt)+soil.totalch4*30/1000;
}
...
*/
  ...
  microbe.netnmin[vt][dm] = microbe.nminxclm(veg.cmnt, dm, pstate[vt][I_AVLW1],
...
//printf("%11.3f\n",microbe.netnmin[vt][dm]);
  ...
  if (ag.state == 0)
  {
    ...
    if (veg.ltrfal[vt][dm].carbon < 0.0) { veg.ltrfal[vt][dm].carbon = 0.1; }

    veg.ltrfal[vt][dm].nitrogen = veg.nfall[veg.cmnt] * pstate[vt][I_STRN];
    if (veg.ltrfal[vt][dm].nitrogen < 0.0) { veg.ltrfal[vt][dm].nitrogen = 0.1; }
...
//printf("%9.3f,%9.3f,%9.3f,%9.3f\n", veg.inprodcn[vt],veg.inuptake[vt],pstate[vt][I_STON],pstate[vt][I_AVLN]);
...
// Changes in available nitrogen in soil
  ...
  if (avlnflag == 1)
  {
    if (ag.state == 0)
    {
      soil.nlost[vt][dm] = pstate[vt][I_AVLN] / pstate[vt][I_SM];
...
// added for 3-box hydrological model
  if (pstate[vt][I_AVLW]+pdstate[vt][I_AVLW] <= 0.0) {
	 pdstate[vt][I_AVLW] = 0.001 - pstate[vt][I_AVLW];
  }

//printf("%9.3f,%9.3f,%9.3f,%d\n",hydm.trans2[vt],soil.rperc[vt][dm],soil.sperc[vt][dm],dm);

  pdstate[vt][I_AVLW1] = soil.snowinf[vt][dm] + hyd.rainthrough[vt][dm] - hydm.lyperc1[vt][dm] - hydm.trans1[vt] - soil.rperc1[vt][dm] - soil.sperc1[vt][dm]-deltafreeze1;
...
// end of changed for available water pool
  ...
  pdstate[vt][I_RGRW] = soil.rperc[vt][dm] - soil.rrun[vt][dm];
  pdstate[vt][I_SNWPCK] = atms.snowfall[vt][dm] - soil.snowinf[vt][dm];
  pdstate[vt][I_SGRW] = soil.sperc[vt][dm] - soil.srun[vt][dm];
  pdstate[vt][I_SM] = pdstate[vt][I_AVLW];
 ...
 // influenced by frozen, decreasing soil available water, Sept/19/2000, Q.Z.
  ...
  pdstate[vt][I_PCTP] = 100.0 * pdstate[vt][I_SM]/soil.totpor[vt];
  ...
  pdstate[vt][I_VSM] = pdstate[vt][I_SM]/(soil.rootz[vt]*1000.0);
  if (pstate[vt][I_VSM]+pdstate[vt][I_VSM] <= 0.0) {
    pdstate[vt][I_VSM] = 0.001 - pstate[vt][I_VSM];
  }

 // added for 3-box hydrology
//  pdstate[I_VSM1] = pdstate[I_SM1]/(soil.dpwbox1*1000.0);
//  if (pstate[I_VSM1]+pdstate[I_VSM1] <= 0.0) {
//	 pdstate[I_VSM1] = 0.001 - pstate[I_VSM1];
//  }
...
// end of adding
  ...
  pdstate[vt][I_RPERC] = soil.rperc[vt][dm];
...
/* ksoil: effect of soil moisture on nitrogen uptake by plants
  ...
  if (moistlim == 0) { ksoil[vt] = pow(vfc,3.0); }
  else { ksoil[vt] = pow(y[vt][I_VSM1],3.0); }
...
// modified for 3-box hydrology, replace y[I_VSM] with y[I_VSM2], Oct 11 2000
  ...
  {
    rhmoist[vt] = (y[vt][I_VSM1]- microbe.moistmin[dcmnt])
              * (y[vt][I_VSM1]-microbe.moistmax[dcmnt]);
//    rhmoist /= rhmoist - pow((y[I_VSM] - microbe.moistopt[dcmnt]),2.0);
    rhmoist[vt] /= rhmoist[vt] - pow((y[vt][I_VSM1] - microbe.moistopt[dcmnt]),2.0);
  }
  if (rhmoist[vt] < 0.0) { rhmoist[vt] = 0.01; }
//printf("%9.3f\n",rhmoist[vt]);
};
...
//  hydm.root3= 1.0-exp(-0.5*hydm.rtindex3+hydm.kwt3);
  ...
  hydm.root2[vt]= 0.20;   // asume 90% root at second root layer
  hydm.root3[vt]= 0.50;   // asume 90% root at second root layer

  hydm.wscal=1.0; // hydm

 // *************** end of changing

 //   veg.plant[dm].carbon = vegca[dcmnt] * psiplusc + vegcb[dcmnt];									//wsr: this is the process for input initial condition values from .dat file
...
//  hyd.hLWP[dm] =  hyd.LWP(soil.avlh2o[dm], soil.awcapmm);
  ...
  hyd.hLWP[vt][dm] =  hyd.LWP(soil.avlh2o2[vt][dm], soil.fldcap[vt], soil.pctsand, soil.pctclay, soil.pctsilt);
...
void TTEM::getsitecd(const int& numcmnt, ofstream& rflog1)
{
  ...
  cout << "Enter name of the site (.ECD) data file with the parameter values  for cmnt (Upland)" << endl;
  rflog1 << "Enter name of the site (.ECD) data file with the parameter values cmnt (Upland)" << endl;
  for (dv = 0; dv < numcmnt; dv++)
  {
    cout << (dv+1) << ": ";
    fpara >> ecd1;

    rflog1 << (dv+1) << ": " << ecd1 << endl;
...
void TTEM::getsitecd_p(const int& numcmnt, ofstream& rflog1)
{

  int dv;
  //char ecd[80];

  cout << "Enter name of the site (.ECD) data file with the parameter values  for cmnt (Peatland)" << endl;
  rflog1 << "Enter name of the site (.ECD) data file with the parameter values cmnt (Peatland)" << endl;
  for (dv = 0; dv < numcmnt; dv++)
  {
    cout << (dv+1) << ": ";
    fpara >> ecd2;

    rflog1 << (dv+1) << ": " << ecd2 << endl;
...
/* ************************wsr*************************************
************************************************************** */
void TTEM::getsitecd_p(const int& dv, char ecd[80])
{
  // Function added by DWK on 20000102
...
// Addition by Q. Z.
  ...
  fecd[dv] >> dummy >> veg.cmnt_p;
  subsist[vt]= veg.cmnt_p;
  veg.cmnt_p= subsist[vt];

  fecd[dv] >> dummy >> veg.cmnt_name_p;
  fecd[dv] >> dummy >> sitename_p;
  fecd[dv] >> dummy >> sitecol_p;
  fecd[dv] >> dummy >> siterow_p;
  fecd[dv] >> dummy >> updated_p;

  fecd[dv] >> dummy >> vegca_p[veg.cmnt];
  fecd[dv] >> dummy >> vegcb_p[veg.cmnt];
  fecd[dv] >> dummy >> strna_p[veg.cmnt];
  fecd[dv] >> dummy >> strnb_p[veg.cmnt];
  fecd[dv] >> dummy >> solca_p[veg.cmnt];
  fecd[dv] >> dummy >> solcb_p[veg.cmnt];
  fecd[dv] >> dummy >> solna_p[veg.cmnt];
  fecd[dv] >> dummy >> solnb_p[veg.cmnt];
  fecd[dv] >> dummy >> avlna_p[veg.cmnt];
  fecd[dv] >> dummy >> avlnb_p[veg.cmnt];
  fecd[dv] >> dummy >> stona_p[veg.cmnt];
  fecd[dv] >> dummy >> stonb_p[veg.cmnt];

  fecd[dv] >> dummy >> veg.unleaf12_p[veg.cmnt];
  // veg.prevleafmx changed to veg.initleafmx by DWK on 20000130
  fecd[dv] >> dummy >> veg.initleafmx_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.cmaxcut_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.cmax1a_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.cmax1b_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.cmax2a_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.cmax2b_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.cfall_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.kra_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.krb_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.kda_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.kdb_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.lcclnc_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.propftos_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.nmaxcut_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.nmax1a_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.nmax1b_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.nmax2a_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.nmax2b_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.nfall_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.nupa_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.nupb_p[veg.cmnt];
  fecd[dv] >> dummy >> soil.nloss_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.nfixpar_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.initcneven_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.cnmin_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.c2na_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.c2nb_p[veg.cmnt];
  fecd[dv] >> dummy >> veg.c2nmin_p[veg.cmnt];
  fecd[dv] >> dummy >> microbe.cnsoil_p[veg.cmnt];
...
void TTEM::transfer(void)

{


//transfer the peatland community data to TEM 
vegca_temp[veg.cmnt] = vegca[veg.cmnt];
vegcb_temp[veg.cmnt] = vegcb[veg.cmnt];
strna_temp[veg.cmnt] = strna[veg.cmnt];
strnb_temp[veg.cmnt] = strnb[veg.cmnt];
solca_temp[veg.cmnt] = solca[veg.cmnt];
solcb_temp[veg.cmnt] = solcb[veg.cmnt];
solna_temp[veg.cmnt] = solna[veg.cmnt];
solnb_temp[veg.cmnt] = solnb[veg.cmnt];
avlna_temp[veg.cmnt] = avlna[veg.cmnt];
avlnb_temp[veg.cmnt] = avlnb[veg.cmnt];
stona_temp[veg.cmnt] = stona[veg.cmnt];
stonb_temp[veg.cmnt] = stonb[veg.cmnt];

vegca[veg.cmnt] = vegca_p[veg.cmnt];
vegcb[veg.cmnt] = vegcb_p[veg.cmnt];
strna[veg.cmnt] = strna_p[veg.cmnt];
strnb[veg.cmnt] = strnb_p[veg.cmnt];
solca[veg.cmnt] = solca_p[veg.cmnt];
solcb[veg.cmnt] = solcb_p[veg.cmnt];
solna[veg.cmnt] = solna_p[veg.cmnt];
solnb[veg.cmnt] = solnb_p[veg.cmnt];
avlna[veg.cmnt] = avlna_p[veg.cmnt];
avlnb[veg.cmnt] = avlnb_p[veg.cmnt];
stona[veg.cmnt] = stona_p[veg.cmnt];
stonb[veg.cmnt] = stonb_p[veg.cmnt];

veg.unleaf12_temp[veg.cmnt] = veg.unleaf12[veg.cmnt];
veg.initleafmx_temp[veg.cmnt] = veg.initleafmx[veg.cmnt];
veg.cmaxcut_temp[veg.cmnt] = veg.cmaxcut[veg.cmnt];
veg.cmax1a_temp[veg.cmnt] = veg.cmax1a[veg.cmnt];
veg.cmax1b_temp[veg.cmnt] = veg.cmax1b[veg.cmnt];
veg.cmax2a_temp[veg.cmnt] = veg.cmax2a[veg.cmnt];
veg.cmax2b_temp[veg.cmnt] = veg.cmax2b[veg.cmnt];
veg.cfall_temp[veg.cmnt] = veg.cfall[veg.cmnt];
veg.kra_temp[veg.cmnt] = veg.kra[veg.cmnt];
veg.krb_temp[veg.cmnt] = veg.krb[veg.cmnt];
microbe.kda_temp[veg.cmnt] = microbe.kda[veg.cmnt];
microbe.kdb_temp[veg.cmnt] = microbe.kdb[veg.cmnt];
microbe.lcclnc_temp[veg.cmnt] = microbe.lcclnc[veg.cmnt];
microbe.propftos_temp[veg.cmnt] = microbe.propftos[veg.cmnt];
veg.nmaxcut_temp[veg.cmnt] = veg.nmaxcut[veg.cmnt];
veg.nmax1a_temp[veg.cmnt] = veg.nmax1a[veg.cmnt];
veg.nmax1b_temp[veg.cmnt] = veg.nmax1b[veg.cmnt];
veg.nmax2a_temp[veg.cmnt] = veg.nmax2a[veg.cmnt];
veg.nmax2b_temp[veg.cmnt] = veg.nmax2b[veg.cmnt];
veg.nfall_temp[veg.cmnt] = veg.nfall[veg.cmnt];
microbe.nupa_temp[veg.cmnt] = microbe.nupa[veg.cmnt];
microbe.nupb_temp[veg.cmnt] = microbe.nupb[veg.cmnt];
soil.nloss_temp[veg.cmnt] = soil.nloss[veg.cmnt];
microbe.nfixpar_temp[veg.cmnt] = microbe.nfixpar[veg.cmnt];
veg.initcneven_temp[veg.cmnt] = veg.initcneven[veg.cmnt];
veg.cnmin_temp[veg.cmnt] = veg.cnmin[veg.cmnt];
veg.c2na_temp[veg.cmnt] = veg.c2na[veg.cmnt];
veg.c2nb_temp[veg.cmnt] = veg.c2nb[veg.cmnt];
veg.c2nmin_temp[veg.cmnt] = veg.c2nmin[veg.cmnt];
microbe.cnsoil_temp[veg.cmnt] = microbe.cnsoil[veg.cmnt];



veg.unleaf12[veg.cmnt] = veg.unleaf12_p[veg.cmnt];
veg.initleafmx[veg.cmnt] = veg.initleafmx_p[veg.cmnt];
veg.cmaxcut[veg.cmnt] = veg.cmaxcut_p[veg.cmnt];
veg.cmax1a[veg.cmnt] = veg.cmax1a_p[veg.cmnt];
veg.cmax1b[veg.cmnt] = veg.cmax1b_p[veg.cmnt];
veg.cmax2a[veg.cmnt] = veg.cmax2a_p[veg.cmnt];
veg.cmax2b[veg.cmnt] = veg.cmax2b_p[veg.cmnt];
veg.cfall[veg.cmnt] = veg.cfall_p[veg.cmnt];
veg.kra[veg.cmnt] = veg.kra_p[veg.cmnt];
veg.krb[veg.cmnt] = veg.krb_p[veg.cmnt];
microbe.kda[veg.cmnt] = microbe.kda_p[veg.cmnt];
microbe.kdb[veg.cmnt] = microbe.kdb_p[veg.cmnt];
microbe.lcclnc[veg.cmnt] = microbe.lcclnc_p[veg.cmnt];
microbe.propftos[veg.cmnt] = microbe.propftos_p[veg.cmnt];
veg.nmaxcut[veg.cmnt] = veg.nmaxcut_p[veg.cmnt];
veg.nmax1a[veg.cmnt] = veg.nmax1a_p[veg.cmnt];
veg.nmax1b[veg.cmnt] = veg.nmax1b_p[veg.cmnt];
veg.nmax2a[veg.cmnt] = veg.nmax2a_p[veg.cmnt];
veg.nmax2b[veg.cmnt] = veg.nmax2b_p[veg.cmnt];
veg.nfall[veg.cmnt] = veg.nfall_p[veg.cmnt];
microbe.nupa[veg.cmnt] = microbe.nupa_p[veg.cmnt];
microbe.nupb[veg.cmnt] = microbe.nupb_p[veg.cmnt];
soil.nloss[veg.cmnt] = soil.nloss_p[veg.cmnt];
microbe.nfixpar[veg.cmnt] = microbe.nfixpar_p[veg.cmnt];
veg.initcneven[veg.cmnt] = veg.initcneven_p[veg.cmnt];
veg.cnmin[veg.cmnt] = veg.cnmin_p[veg.cmnt];
veg.c2na[veg.cmnt] = veg.c2na_p[veg.cmnt];
veg.c2nb[veg.cmnt] = veg.c2nb_p[veg.cmnt];
veg.c2nmin[veg.cmnt] = veg.c2nmin_p[veg.cmnt];
microbe.cnsoil[veg.cmnt] = microbe.cnsoil_p[veg.cmnt];

}


void TTEM::recover(void)
{

vegca[veg.cmnt] = vegca_temp[veg.cmnt];
vegcb[veg.cmnt] = vegcb_temp[veg.cmnt];
strna[veg.cmnt] = strna_temp[veg.cmnt];
strnb[veg.cmnt] = strnb_temp[veg.cmnt];
solca[veg.cmnt] = solca_temp[veg.cmnt];
solcb[veg.cmnt] = solcb_temp[veg.cmnt];
solna[veg.cmnt] = solna_temp[veg.cmnt];
solnb[veg.cmnt] = solnb_temp[veg.cmnt];
avlna[veg.cmnt] = avlna_temp[veg.cmnt];
avlnb[veg.cmnt] = avlnb_temp[veg.cmnt];
stona[veg.cmnt] = stona_temp[veg.cmnt];
stonb[veg.cmnt] = stonb_temp[veg.cmnt];

veg.unleaf12[veg.cmnt] = veg.unleaf12_temp[veg.cmnt];
veg.initleafmx[veg.cmnt] = veg.initleafmx_temp[veg.cmnt];
veg.cmaxcut[veg.cmnt] = veg.cmaxcut_temp[veg.cmnt];
veg.cmax1a[veg.cmnt] = veg.cmax1a_temp[veg.cmnt];
veg.cmax1b[veg.cmnt] = veg.cmax1b_temp[veg.cmnt];
veg.cmax2a[veg.cmnt] = veg.cmax2a_temp[veg.cmnt];
veg.cmax2b[veg.cmnt] = veg.cmax2b_temp[veg.cmnt];
veg.cfall[veg.cmnt] = veg.cfall_temp[veg.cmnt];
veg.kra[veg.cmnt] = veg.kra_temp[veg.cmnt];
veg.krb[veg.cmnt] = veg.krb_temp[veg.cmnt];
microbe.kda[veg.cmnt] = microbe.kda_temp[veg.cmnt];
microbe.kdb[veg.cmnt] = microbe.kdb_temp[veg.cmnt];
microbe.lcclnc[veg.cmnt] = microbe.lcclnc_temp[veg.cmnt];
microbe.propftos[veg.cmnt] = microbe.propftos_temp[veg.cmnt];
veg.nmaxcut[veg.cmnt] = veg.nmaxcut_temp[veg.cmnt];
veg.nmax1a[veg.cmnt] = veg.nmax1a_temp[veg.cmnt];
veg.nmax1b[veg.cmnt] = veg.nmax1b_temp[veg.cmnt];
veg.nmax2a[veg.cmnt] = veg.nmax2a_temp[veg.cmnt];
veg.nmax2b[veg.cmnt] = veg.nmax2b_temp[veg.cmnt];
veg.nfall[veg.cmnt] = veg.nfall_temp[veg.cmnt];
microbe.nupa[veg.cmnt] = microbe.nupa_temp[veg.cmnt];
microbe.nupb[veg.cmnt] = microbe.nupb_temp[veg.cmnt];
soil.nloss[veg.cmnt] = soil.nloss_temp[veg.cmnt];
microbe.nfixpar[veg.cmnt] = microbe.nfixpar_temp[veg.cmnt];
veg.initcneven[veg.cmnt] = veg.initcneven_temp[veg.cmnt];
veg.cnmin[veg.cmnt] = veg.cnmin_temp[veg.cmnt];
veg.c2na[veg.cmnt] = veg.c2na_temp[veg.cmnt];
veg.c2nb[veg.cmnt] = veg.c2nb_temp[veg.cmnt];
veg.c2nmin[veg.cmnt] = veg.c2nmin_temp[veg.cmnt];
microbe.cnsoil[veg.cmnt] = microbe.cnsoil_temp[veg.cmnt];
...
//  if (y[I_VSM1] != y[I_SM1] / (soil.dpwbox1 * 1000.0))
//  {
//    y[I_VSM1] = y[I_SM1] / (soil.dpwbox1 * 1000.0);
//    if (y[I_VSM1] <= 0.0) { y[I_VSM1] = 0.001; }
//  }

//   if (y[I_VSM2] != y[I_SM2] / (soil.rootmx * 1000.0-soil.dpwbox1*1000.0))
...
void TTEM::monthxclm(const int& dcmnt, double& tgppopt, const int& dm, const int& vt)
{

double raq10;

tgppopt = veg.toptmin[dcmnt];
...
void TTEM::setELMNTecd(const int& kdinflg, const int& dcmnt, const double& psiplusc, const int& vt)      // call only once in equilibrium
{
...
//   and veg.topt
 ...
 //     {
        ...
        {
	//       veg.topt[vt] = tair[vt][i];
	veg.prvleafmx[dcmnt]=veg.unnormleaf[vt][i];
	     }
 //     }
    }
...
void TTEM::setMonth(const int& dm, double y[NVT][NUMEQ], const int& vt)
{
  ...
  soil.moist1[vt][dm] = y[vt][I_SM1];
  soil.moist2[vt][dm] = y[vt][I_SM2];
  soil.moist3[vt][dm] = y[vt][I_SM3];
  ...
  soil.vsm1[vt][dm] = y[vt][I_SM1]/(soil.dpwbox1[vt]*1000.0);
  soil.vsm2[vt][dm] = y[vt][I_SM2]/(soil.dpwbox2[vt]*1000.0);
  soil.vsm3[vt][dm] = y[vt][I_SM3]/(soil.dpwbox3[vt]*1000.0);
  ...
  for (dm = 0; dm < CYCLE; dm++)
  {
    if (initFlag == 1)
    {
      ...
      if (atms.tprecflag != 0) { atms.prec[vt][dm] = atms.tprec[dyr][dm]; }
      	...
      	if (atms.RGflag == 0)
      	{																			//this part does the calculation from RG to PAR if RG is the input data (replace the potclm in telmnt423e.cpp)
      //atms.nirr[vt][dm] = atms.tclds[dyr][dm];
      
     //atms.girr[vt][dm] = atms.xgirr(atms.latt,dm,0.0); 
      
     //atms.clds[vt][dm] = atms.mkclds(atms.girr[vt][dm], atms.nirr[vt][dm]);
      
     //atms.par[vt][dm]  = atms.xpar(atms.clds[vt][dm], atms.nirr[vt][dm]); 
     
     atms.par[vt][dm] = atms.tclds[dyr][dm];                                                                          //this part does the direct input from PAR of TraCE 21000 data
     atms.nirr[vt][dm] = atms.par[vt][dm] / 0.49;
      }
      else 
      {
       atms.clds[vt][dm] = atms.tclds[dyr][dm];											
       
       atms.girr[vt][dm] = atms.xgirr(atms.latt,dm,0.0); 
       
       atms.nirr[vt][dm] = atms.xnirr(atms.clds[vt][dm], atms.girr[vt][dm]);
...
sthermal.water2 = soil.vsm2[vt][dm];

	sthermal.SoilThermal(&sthermal.water2, &sthermal.calcu_cdsnow, &sthermal.airt19, &sthermal.airt29, &sthermal.airt39, &sthermal.hsnow19, &sthermal.hsnow29,
...
//printf("%f\n",veg.cfall[veg.cmnt]);
	...
	int z = 0.0;
				...
				for(int i = 0;i < ceil(soil.sum_depth / 10);i++)
				{
					z = i+1;
                             if( i>= 200) 
                                    {
                                    soil.intersoilt[i] = soil.intersoilt[190];;
                                    }
...
int TTEM::transient(const int& dyr, const int& itype, double& tol, const int& RTIME, const int& vt,int& init_carbonflag)                //the step here is year
{


if (soil.init_carbonflag == 1)      { soil.init_carbon[0] = soil.org[vt][0].carbon; }   											//wsr: input initial soil carbon storage to trasient step

soil.callflag ++;

//if (soil.callflag == 10)   {  soil.delta_c = soil.org[vt][0].carbon - soil.init_carbon[0] ;          

//soil.init_carbon[0] = soil.org[vt][0].carbon;   //wsr: set delta soil storage in 10 years carbon for bulk density calling

//soil.callflag = 0;                                                // wsr: reset calling flag

//if (soil.org[vt][0].carbon > 25000)
//{
//soil.delta_c1 = soil.delta_c;
//soil.bulkdensity(soil.delta_c1,soil.sum_depth);
//}
//}

//printf("%9d,%9.3f,%9.3f,%9.8f\n",soil.callflag, soil.org[vt][0].carbon,soil.delta_c,soil.sum_depth);
  endeq = 0;
...
if (atms.RGflag == 0)
      	{																			//this part does the calculation from RG to PAR if RG is the input data (replace the potclm in telmnt423e.cpp)
      //atms.nirr[vt][dm] = atms.tclds[dyr][dm]/1;
      
     //atms.girr[vt][dm] = atms.xgirr(atms.latt,dm,365); 
     ...
     atms.nirr[vt][dm] = atms.par[vt][dm] / 0.49;
      ...
      // printf("%9.3f, %9.3f,    %9.3f\n",atms.par[vt][dm],atms.nirr[vt][dm],atms.clds[vt][dm]);
      
  //    printf("%9.3f,%9.3f,%d\n",atms.par[vt][dm],atms.girr[vt][dm],dyr);
    //  printf("%13.3f,%13.3f\n",atms.par[vt][dm],atms.tclds[122][dm]);
     //added for hydrology model by QZ
      if (hyd.vapflag != 0) { hyd.vap[vt][dm] = hyd.tvap[dyr][dm]; }
...
/*****************************************************************************************************************/
  //wsr: add from TEM5.0 determine the wind speed level
     ...
     if (CDM < 50.0) snow_class = 6; // class Ephemeral
       ...
       {
        if (CDM > 125.0) {
                         if (atms.prec[vt][dm] / atms.daze[dm] >2.0)
                           {
...
/************************************************************/
     ...
     switch (dm) {
     case 0: sthermal.hsnow19=(soil.snowpack[vt][CYCLE-1]+soil.snowpack[vt][0])/2.0/1000.0/snow_dens; // satisfy soil thermal model
             sthermal.hsnow29= soil.snowpack[vt][0]/1000.0/snow_dens;
             sthermal.hsnow39= (soil.snowpack[vt][1]+soil.snowpack[vt][0])/2.0/1000.0/snow_dens;
             break;
     case 11: sthermal.hsnow19=(soil.snowpack[vt][11]+soil.snowpack[vt][10])/2.0/1000.0/snow_dens; // satisfy soil thermal model
              sthermal.hsnow29= soil.snowpack[vt][11]/1000.0/snow_dens;
              sthermal.hsnow39= (soil.snowpack[vt][11]+soil.snowpack[vt][0])/2.0/1000.0/snow_dens;
            break;
     default: sthermal.hsnow19=(soil.snowpack[vt][dm-1]+soil.snowpack[vt][dm])/2.0/1000.0/snow_dens; // satisfy soil thermal model
              sthermal.hsnow29= soil.snowpack[vt][dm]/1000.0/snow_dens;
              sthermal.hsnow39= (soil.snowpack[vt][dm]+soil.snowpack[vt][dm+1])/2.0/1000.0/snow_dens;
            break;
               }

     sthermal.SoilThermal(&sthermal.water2, &sthermal.calcu_cdsnow, &sthermal.airt19, &sthermal.airt29, &sthermal.airt39, &sthermal.hsnow19, &sthermal.hsnow29, &sthermal.hsnow39,sthermal.weight9, &sthermal.smass9, &sthermal.is9, &sthermal.ism19, sthermal.x9, sthermal.dx9, sthermal.xfb9, sthermal.xfa9,
		sthermal.water9, sthermal.t9, &sthermal.tsoil,&sthermal.frontd,&sthermal.thawbegin,&sthermal.thawend,sthermal.diffsoilt,veg.cmnt);
...
//printf("%9.3f,%9.3f\n", atms.tair[vt][dm],veg.topt[vt]);
  ...
  // soil.cal_diff(soil.vsm1[vt][dm],soil.vsm2[vt][dm],soil.dpwbox1[vt],soil.dpwbox2[vt],vt,dm,soil.watertable,atms.tair[vt][dm],soil.sum_depth);
